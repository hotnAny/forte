...
'iterations.'
    ...
    def update_desvars_oc(self):
        ...
        while (lam2 - lam1) / (lam2 + lam1) > 1e-8 and lam2 > 1e-40:
            ...
            if self.pasv.any() or self.actv.any():
                ...
                # min_gradient = 0.001;
                try:
                    self.gradientprinted
                except:
                    print '[xac] again, gradient is', self.gradient
                    print '[xac] and, slope is ', slope
                    self.gradientprinted = True
                ...
                #                 gradient = self.gradient
                #                 tr_df = self.sigmoid(df_val, slope, gradient)
                #                 tr_min = self.sigmoid(0, slope, gradient)
                #                 tr_max = self.sigmoid(1, slope, gradient)
                        ...
                        for j in xrange(0, self.nely):
                            for i in xrange(0, self.nelx):
                                ...
                                try:
                                    ...
                                    # gradient = self.gradient # * math.exp(stress)
    ...
    def get_distance_field(self, elms, elm_vals, nelx, nely):
        ...
        tr_min = self.sigmoid(0, self.slope, self.gradient)
        tr_max = self.sigmoid(1, self.slope, self.gradient)
        ...
        for i in xrange(0, nelx):
            for j in xrange(0, nely):
                ...
                df[i][j] = self.sigmoid(df[i][j], self.slope, self.gradient)
                df[i][j] = (tr_max-df[i][j])/(tr_max-tr_min)
                df[i][j] = 0 if df[i][j] < eps else df[i][j]

        # debug, print sigmoided, normalized distance field
        for i in xrange(0, nelx):
            print [(format(x, '1.1f') if x > eps else '   ') for x in df[i]]
        return df

    # 1 - (1/(1+e^(64*(0.1-x)))-0.039)/(1-0.039)
    def sigmoid(self, t, slope, gradient):
        return 1 / (1 + math.exp(slope*(gradient-t)))
