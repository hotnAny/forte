...
from nodenums import node_nums_2d
# from nodenums import elm_num_2d
...
def optimize(tpd):
    return call_topy(tpd, OPTIMIZATION, True)
...
if __name__ == "__main__":
    ...
    tpd, debug_voxelgrid = gen_tpd(design, resolution, material, field_intensity, weight)
    print ''.join(debug_voxelgrid)[::-1]
    probname = optimize(tpd)
    sub_outpath = args.path + '_' + str(long(time.time())) + '_' + \
        str(resolution) + '_' + str(material) + '_' + str(gradient)
    print sub_outpath
